...
contract('DXC functionailities', async (accounts) => {
    ...
    it('Blacklisting should work', async () => {
        ...
        const blackListErr = await proxiedDxc
            .addToBlackList(accounts[0])
            .catch(err => err);
        assert.equal('Error: VM Exception while processing transaction: revert Owner cannot be blacklisted', String(blackListErr));
